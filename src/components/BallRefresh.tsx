...
const BallRefresh: React.FC<BallRefreshProps> = ({ scale, position, rotation }) => {
    const texture = useTexture("/textures/ballRefreshUvMap.jpg")
    // Hover state
      ...
      <group dispose={null} scale={scale} position={position} rotation={rotation} >
        <group>
          <mesh receiveShadow castShadow onPointerOver={onPointerOver} onPointerOut={onPointerOut} 
            onClick={() => window.open('https://newgallery.collageofmyself.com/', '_blank')}>
