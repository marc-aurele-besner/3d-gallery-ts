...
const BallOpenSea: React.FC<BallOpenSeaProps> = ({ scale, position, rotation }) => {
    const texture = useTexture("/textures/ballOpenSeaUvMap.jpg")
    // Hover state
      ...
      <group dispose={null} scale={scale} position={position} rotation={rotation} >
        <group>
          <mesh receiveShadow castShadow onPointerOver={onPointerOver} onPointerOut={onPointerOut} 
            onClick={() => window.open('https://opensea.io/collection/collage-of-myself', '_blank')}>
