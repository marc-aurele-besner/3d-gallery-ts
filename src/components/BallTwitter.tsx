...
const BallTwitter: React.FC<BallTwitterProps> = ({ scale, position, rotation }) => {
    const texture = useTexture("/textures/ballTwitterUvMap.jpg")
    // Hover state
      ...
      <group dispose={null} scale={scale} position={position} rotation={rotation} >
        <group>
          <mesh receiveShadow castShadow onPointerOver={onPointerOver} onPointerOut={onPointerOut} 
            onClick={() => window.open('https://twitter.com/collageofmyself', '_blank')}>
